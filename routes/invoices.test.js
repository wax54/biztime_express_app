...
beforeAll( async () => {
    ...
    try{
        ...
        console.log('apple or ibm already in companies db');
    }
});
...
beforeEach(async () => {
    for(let i in invoices){
        ...
        VALUES($1, $2, $3, $4) RETURNING *`, [inv.comp_code, inv.amt, inv.paid, inv.paid_date]);
...
describe('PUT /invoices/:id', () => {
    ...
    test('throws 404 if no item', async () => {
        const res = await request(app).get('/invoices/nadabing');
        expect(res.statusCode).toBe(404);
    })
});
